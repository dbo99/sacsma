...
prcp <- data.frame(stcroix$prcp.grid[1]) 
names(prcp)[1]<-"prcp_mm"

temp <- data.frame(stcroix$tavg.grid[1])
names(temp)[1]<-"temp_c"

pet <- data.frame(stcroix$prcp.grid[15]) 
pet <- pet/15

names(pet)[1]<-"pet_mm"

ini.states <-  c(0,0,500,500,500,0)
sacSma(par, ini.states, prcp, pet)

par <- c("uztwm", "uzfwm", "lztwm", "lzpfm", "lzfsm", "uzk", "lzpk", "lzsk", 
...
sacSma <- function(par, ini.states = c(0,0,500,500,500,0), prcp, pet) {
  ...
  uztwm  <-  5     #par[1]    # Upper zone tension water capacity [mm]
  uzfwm  <-  4     #par[2]    # Upper zone free water capacity [mm]
  lztwm  <-  3     #par[3]    # Lower zone tension water capacity [mm]
  lzfpm  <-  5     #par[4]    # Lower zone primary free water capacity [mm]
  lzfsm  <-  5     #par[5]    # Lower zone supplementary free water capacity [mm]
  uzk    <-  1     #par[6]    # Upper zone free water lateral depletion rate [1/day]
  lzpk   <-  1     #par[7]    # Lower zone primary free water depletion rate [1/day]
  lzsk   <-  1     #par[8]    # Lower zone supplementary free water depletion rate [1/day]
  zperc  <-  1     #par[9]    # Percolation demand scale parameter [-]
  rexp   <-  1     #par[10]   # Percolation demand shape parameter [-]
  pfree  <-  0.1   #par[11]   # Percolating water split parameter (decimal fraction)
  pctim  <-  0.05  #par[12]   # Impervious fraction of the watershed area (decimal fraction)
  adimp  <-  0.05  #par[13]   # Additional impervious areas (decimal fraction)
  riva   <-  0.1   #par[14]   # Riparian vegetation area (decimal fraction)
  side   <-  0.4   #par[15]   # The ratio of deep recharge to channel base flow [-]
  rserv  <-  0.1   #par[16]   # Fraction of lower zone free water not transferrable (decimal fraction)
